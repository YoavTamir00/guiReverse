...
/**
 * Due to limitation of Scene Builders, We preferred to write this class hard coded
 * on our own. The rest of the scenes are read from xml files.
 */
public class GameScreen extends HBox implements ReversiUi {
    ...
    public GameScreen(VBox root) {
        super();
        this.settings = VisualSettingsFromFile.getInstance();
        resetValues();
        this.initEmptyBoard();
    ...
    private void initEmptyBoard() {
    	int dimension = (int) settings.boardDimension();
    ...
    private void drawCell(int x, int y, Color color) {
        ...
        gc.fillRoundRect(x * squareSize, y * squareSize, squareSize, squareSize, squareSize, squareSize);
        drawGrid();
    }
        ...
        for (int x = 0; x < board.length; x++) {
            for (int y = 0; y < board.length; y++) {
                if (board[x][y] != Tile.Empty) {
                    if (settings.startingPlayer() == Tile.X) {
                        drawCell(y, x, board[x][y] == Tile.X ? p1Color : p2Color);
                    } else {
