...
public class GameFlow {
    ...
    public void runGame(Board board) {
        // 'i' will be 1 if X is playing and will be -1 if P is playing.
        // If we want to switch the turn from X to O, all we have to do now is
        // multiply i by -1.
        int i;
        // Checking which player holds the X tile, he starts.
        ...
        while (done == GameState.playing) {
            ...
            // no possible moves for the current player, continue to the next player.
            if (possible.size() == 0) {
                i *= -1;
                gui.noAvailableMoves(currentPlayer.getName());
                // check if the game is over(if BOTH players don't have moves).
        ...
        // Updating the GUI according to the latest board-information.
        gui.setP1Score(board.countTile(p1.getTile()));
        gui.setP2Score(board.countTile(p2.getTile()));
        gui.printCurrentBoard();
