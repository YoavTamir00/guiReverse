...
public class NormalRules implements GameRules {
	...
	public GameState gameOver(Board board) {
	    ...
	    if (board.getNumTaken() != dimension * dimension &&
	        (movesX.size() != 0 || movesO.size() != 0)) {
	        // The game is on.
	...
	private void cellsToFlipInOneDirection(CellCollection ans,Board board, Cell cell, Tile player, int deltaRow,
	    ...
	    for (i = row + deltaRow, j = column + deltaColumn;
	         i >= 0 && j >= 0 && i < dimension && j < dimension; i += deltaRow, j += deltaColumn) {
	        current = b[i][j];
